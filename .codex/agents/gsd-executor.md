...
@D:/latuan/Programming/dich-truyen-agent/.codex/get-shit-done/references/thinking-models-execution.md

**iOS app scaffolding:** If this plan creates an iOS app target, follow ios-scaffold guidance:
@D:/latuan/Programming/dich-truyen-agent/.codex/get-shit-done/references/ios-scaffold.md
...
@D:/latuan/Programming/dich-truyen-agent/.codex/get-shit-done/references/executor-examples.md
</deviation_rules>
...
**See @D:/latuan/Programming/dich-truyen-agent/.codex/get-shit-done/references/checkpoints.md**
...
**When the orchestrator passes both `MVP_MODE=true` and `TDD_MODE=true`:** Before running the implementation step of any task with `tdd="true"`, run the runtime gate from `@D:/latuan/Programming/dich-truyen-agent/.codex/get-shit-done/references/execute-mvp-tdd.md`. If the gate trips, halt and report — do NOT proceed to the implementation step.
...
**Codex child-agent approval boundary (MANDATORY):**

When running as a Codex child executor, do **not** call a shell tool with
`sandbox_permissions="require_escalated"`. Nested child-agent approval requests are not
reliably forwarded to the parent IDE thread and can leave the executor waiting silently.

If a required command fails because sandbox ACLs or network restrictions require escalation:

1. STOP immediately. Do not retry with `require_escalated`.
2. Return `## CHECKPOINT REACHED` with type `human-action`.
3. Include the exact single command the parent orchestrator must run with escalation.
4. After the parent completes that command, continue in a fresh executor.

For Git commits, run `git add` and `git commit` as separate commands. Never bundle staging,
commit, status, rev-parse, or diff checks into one shell chain. If either command fails with
`.git/index.lock` permission denied, return the checkpoint immediately.
...
**Use template:** @D:/latuan/Programming/dich-truyen-agent/.codex/get-shit-done/templates/summary.md
