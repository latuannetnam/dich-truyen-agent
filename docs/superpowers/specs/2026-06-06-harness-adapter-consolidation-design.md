...
### 6.2 Partially Shared Translation Skill
...
### 6.3 Shared Agent Bodies With Harness Wrappers
...
For normal use, each harness discovers and calls its own committed prefixed adapters. Users call `ag-crawl-book`, `cc-translate-book`, `oc-check-translation`, or `codex-export-book` based on the active harness.
...
- `AGENTS.md` points users to prefixed skills and explains that runtime adapters are generated.
...
Documentation should update examples in `AGENTS.md`, `README.md`, and any relevant tests/spec references to the new prefixed names.
...
1. Canonical source files exist for shared skills, shared agent bodies, translation dispatch variants, and guardrail policy.
...
10. `AGENTS.md` documents the generated adapter model and the prefixed entry points.

## 13. Spec Self-Review

- Placeholder scan: clean. No unresolved placeholder markers.
- Internal consistency: the architecture matches the approved generator approach, committed adapters, harness prefixes, all four harness targets, and removal of old unprefixed runtime files.
- Scope check: focused on skill/agent adapter consolidation, generation, discovery safety, and guardrails. Runtime translation CLI behavior remains out of scope.
- Ambiguity check: clarified that only translation dispatch is substantially harness-specific; CLI-only skills render from a shared body with wrapper substitutions.
