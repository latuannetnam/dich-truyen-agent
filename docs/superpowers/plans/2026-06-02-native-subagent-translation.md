# Native Antigravity Subagent Translation Skill Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Rewrite `.agent/skills/translate-book/SKILL.md` to strictly enforce native translation orchestration using the `invoke_subagent` tool.

**Architecture:** Update the skill guide to detail gate checks, path resolution, native subagent tool signature, a detailed translation prompt (incorporating the Lexical Sandbox banned words table), lightweight preview checks, promotion commands, and failure handling.

**Tech Stack:** Markdown, Antigravity Agent Skill.

---

### Task 1: Update Translate-Book Skill File
...
git commit -m "feat(skills): improve translate-book skill to strictly use native subagents"
```
