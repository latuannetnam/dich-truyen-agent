...
2. **Diagnose and Resolve Findings**:
   Inspect findings reported in the terminal or `reports/qa-report.yaml`:
   - **Structural Findings:** Fix any missing chapters, empty files, or state inconsistencies in `state.yaml`.
   - **Chinese Residue Warnings:** Highlighted CJK characters or Chinese punctuation marks remaining in Vietnamese text. Manually clean these lines in the translation files.
